...
import type { TransferIntent } from "./operator"; // Import the TransferIntent type
...
export type PayerClientConfig = {
  ...
  payerAccount: Address; // The payer's wallet address
  signer: WalletClient; // The payer's WalletClient for sending transactions
};
...
export type PayerClientError =
  ...
  | { type: "INSUFFICIENT_ALLOWANCE"; message: string; required: bigint; current: bigint; token: Address }
...
export const createPayerClient = (config: PayerClientConfig) => {
  ...
  if (!config.chain || !config.transportUrl || !config.transfersContractAddress || !config.payerAccount || !config.signer) {
  ...
  const checkAllowance = async (
    ...
    try {
      ...
      const currentAllowance = await publicClient.readContract({
          ...
          args: [config.payerAccount, config.transfersContractAddress],
   ...
   const approveToken = async (
     ...
     if (amount <= 0n) {
         return err({type: "INVALID_INTENT", message: "Approval amount must be positive."});
     }
     try {
        const approveParams: WriteContractParameters<typeof erc20Abi, 'approve'> = {
            ...
            account: config.payerAccount,
            chain: config.chain,
        };
         const txHash = await config.signer.writeContract(approveParams);
  ...
  const transferTokenPreApproved = async (
    signedIntent: TransferIntent
  ): Promise<Result<Hex, PayerClientError>> => {
    // --- Basic Intent Validation ---
    if (!signedIntent || !signedIntent.signature) {
      return err({ type: "INVALID_INTENT", message: "Signed intent is missing or lacks a signature." });
    }
    if (signedIntent.recipientCurrency === '0x0000000000000000000000000000000000000000') {
        return err({ type: "INVALID_INTENT", message: "transferTokenPreApproved requires a token, not native currency." });
    }
    ...
    try {
      ...
      const intentForContract = signedIntent as Required<TransferIntent>;

      // Prepare the arguments for the contract call. Viem expects the struct as an object.
      const args: [Required<TransferIntent>] = [
         intentForContract
      ];

      const writeParams: WriteContractParameters<typeof COMMERCE_ABI, 'transferTokenPreApproved'> = {
            ...
            account: config.payerAccount,
            chain: config.chain,
            // gas, value, etc., could be added here if needed, otherwise viem estimates
      };

      const txHash = await config.signer.writeContract(writeParams);
