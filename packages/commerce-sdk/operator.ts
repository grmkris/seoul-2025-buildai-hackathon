...
export const createOperatorClient = (config: OperatorClientConfig) => {
  ...
  if (!config.chain || !config.transportUrl || !config.transfersContractAddress || !config.operatorAccount || !config.signer) {
  ...
  const isOperatorRegistered = async (): Promise<Result<boolean, OperatorClientError>> => {
     try {
  ...
  const isIntentProcessed = async (intentId: Hex): Promise<Result<boolean, OperatorClientError>> => {
     if (!intentId || intentId.length !== 34) { // Basic validation for bytes16 hex
         return err({ type: "INVALID_INTENT", message: "Invalid intent ID format."});
     }
     try {
      const isProcessed = await publicClient.readContract({
        ...
        args: [config.operatorAccount, intentId],
      });
      if (isProcessed) {
        // Optionally return a specific error if already processed
        return err({ type: "INTENT_ALREADY_PROCESSED", message: "Intent already processed", id: intentId });
      }
  ...
  const createTransferIntentHash = (intent: Omit<TransferIntent, 'signature'>, payerAddress: Address): Hex => {
      ...
      if (intent.prefix) {
          ...
          const prefixedHash = keccak256(encodePacked(['string', 'bytes32'], [intent.prefix, hash]));
          return prefixedHash;
      }
  ...
  const createAndSignTransferIntent = async (
      ...
      if (intent.deadline <= BigInt(Math.floor(Date.now() / 1000))) {
          return err({ type: "INVALID_INTENT", message: "Intent deadline is in the past." });
      }
      if (!intent.id || intent.id.length !== 34) { // Basic validation for bytes16 hex
          return err({ type: "INVALID_INTENT", message: "Invalid intent ID format."});
      }
       if (!payerAddress || !payerAddress.startsWith('0x')) {
          return err({ type: "INVALID_INTENT", message: "Invalid payer address provided." });
      }
      ...
      try {
          ...
          const signature = await (config.signer as WalletClient).signMessage({
          ...
          const signedIntent: TransferIntent = { ...intent, signature };
  ...
  const registerOperator = async (): Promise<Result<Hex, OperatorClientError>> => {
      try {
          const txHash = await (config.signer as WalletClient).writeContract({
              address: config.transfersContractAddress,
              abi: COMMERCE_ABI,
              functionName: "registerOperator",
              account: config.operatorAccount, // Make sure account is linked to signer
              chain: config.chain,
          });
          // Optionally wait for transaction receipt here
          return ok(txHash);
      } catch (error) {
          return err({ type: "CONTRACT_WRITE_ERROR", message: "Failed to register operator", cause: error });
      }
  };

  const registerOperatorWithFeeDestination = async (feeDestination: Address): Promise<Result<Hex, OperatorClientError>> => {
       if (!feeDestination || !feeDestination.startsWith('0x')) { // Basic validation
          return err({ type: "INVALID_INTENT", message: "Invalid fee destination address." });
      }
       try {
          const txHash = await (config.signer as WalletClient).writeContract({
              address: config.transfersContractAddress,
          ...
          return err({ type: "CONTRACT_WRITE_ERROR", message: "Failed to register operator with fee destination", cause: error });
      }
  };

  const unregisterOperator = async (): Promise<Result<Hex, OperatorClientError>> => {
       try {
          const txHash = await (config.signer as WalletClient).writeContract({
              address: config.transfersContractAddress,
              abi: COMMERCE_ABI,
          ...
          return err({ type: "CONTRACT_WRITE_ERROR", message: "Failed to unregister operator", cause: error });
      }
  };
