...
export const generateItemImage = async (props: {
  aiClient: AiClient;
  itemName: string;
  itemDescription: string;
}) => {
  const { aiClient, itemName, itemDescription } = props;

  // Create a detailed prompt for the item image
  const prompt = `Create a detailed fantasy-style image of a magical item called "${itemName}". 
  Description: ${itemDescription}
  The image should be well-lit, detailed, and in a style appropriate for a fantasy RPG game.`;
