...
import { tool } from "ai";
import { z } from "zod";
import type { ConversationId, PaymentIntentId, UserId } from "typeid";
import { DB_SCHEMA } from "@/db/db";
import { usersTable } from "@/db/schema/chat/chat.db";
...
export const createPaymentAgentTools = (props: {
	logger: Logger;
	db: db;
	userId: UserId; // Assuming UserId is passed down or available
	conversationId: ConversationId; // Assuming ConversationId is passed down or available
}) => {
	const { logger, db, userId, conversationId } = props;
	...
	const requestPaymentTool = tool({
		...
		execute: async ({ amount, currencySymbol, reason }) => {
			try {
				...
				const payer = await db.query.usersTable.findFirst({
        ...
        if (!payerAddress) {
          ...
          return { error: "User has no wallet address" };
				...
				const signedIntentResult = await operatorClient.createAndSignTransferIntent(
					intentToSign,
				...
				const paymentIntent = await db.insert(DB_SCHEMA.paymentIntents).values({
					intentId: signedIntent.id,
					conversationId: conversationId, // Passed in props
					userId: userId, // Passed in props
					status: "pending",
        ...
        if (!paymentIntentDbId) {
          ...
          return { error: "Failed to store payment intent" };
				...
				return { error: "An unexpected error occurred while processing the payment request." };
			}
		},
	});
  ...
  const validatePaymentTool = tool({
    ...
    parameters: z.object({
      paymentIntentId: z.string().describe("The ID of the payment intent to validate"),
    }),
    execute: async ({ paymentIntentId }) => {
      try {
        ...
        await db.update(DB_SCHEMA.paymentIntents).set({
          status: "completed",
        }).where(eq(DB_SCHEMA.paymentIntents.id, paymentIntentId as PaymentIntentId));
        ...
        return { error: "An unexpected error occurred while processing the payment validation." };
      }
    },
  });
