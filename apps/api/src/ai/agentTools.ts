...
import { createOgrodjeClient, EventsQueryParamsSchema, MeetupEventsQueryParamsSchema, MeetupsQueryParamsSchema, MeetupEventsPathParamsSchema } from "ogrodje-client";
import { z } from "zod";
...
export const createOgrodjeClientTools = (props: {
  ...
  const eventsTool = tool({
    ...
    execute: async (params) => {
      ...
      if (result.isErr()) {
        props.logger.error({ msg: "Error fetching events:", error: result.error });
  ...
  const meetupsTool = tool({
    ...
    execute: async (params) => {
      ...
      if (result.isErr()) {
        props.logger.error({ msg: "Error fetching meetups:", error: result.error });
  ...
  const meetupEventsTool = tool({
    ...
    execute: async ({ path, query }) => {
      ...
      if (result.isErr()) {
        props.logger.error({ msg: "Error fetching meetup events:", error: result.error });
        return { error: `Failed to fetch meetup events: ${result.error.message}` };
      }
      return result.value;
    }
  })
  ...
  const timelineTool = tool({
    ...
    execute: async () => {
      ...
      if (result.isErr()) {
        props.logger.error({ msg: "Error fetching timeline:", error: result.error });
